...
def process_markdown_files(
    ...
    """处理目录中的所有Markdown文件，每个文件生成单独的JSONL"""
    ...
    for md_file in sorted(input_path.glob('*.md')):
        ...
        print(f'  生成 {len(chunks)} 个块 → {output_file.name}')
    
    # 计算总体统计
    if stats['total_chunks'] > 0:
        # 计算所有块的平均大小
        total_chars = sum(
            f['chunks'] * f['avg_size'] for f in stats['files']
        )
        stats['avg_chunk_size'] = total_chars / stats['total_chunks']
...
def main():
    ...
    parser.add_argument(
        '--output_dir',
        type=str,
        default='kb/chunks_semantic_per_file',
        help='输出目录（默认: kb/chunks_semantic_per_file）'
    )
    parser.add_argument(
        '--chunk_size',
        type=int,
